...
fn dfs(start: Position, map : &mut Vec<Vec<Place>>) -> i32
{
    ...
    while !stack.is_empty()
    {
        ...
        if (current.x as i32)-1 >= 0 && !map[current.x-1][current.y].visited && map[current.x-1][current.y].value == current_value+1
        {
            stack.push(Position{x: current.x-1, y:current.y});
        }
        if current.x+1 < map.len() && !map[current.x+1][current.y].visited && map[current.x+1][current.y].value == current_value+1
        {
            stack.push(Position{x: current.x+1, y:current.y});
        }

        if (current.y as i32)-1 >= 0 && !map[current.x][current.y-1].visited && map[current.x][current.y-1].value == current_value+1
        {
            stack.push(Position{x: current.x, y:current.y-1});
        }
        if current.y+1 < map[current.x].len() && !map[current.x][current.y+1].visited && map[current.x][current.y+1].value == current_value+1
        {
            stack.push(Position{x: current.x, y:current.y+1});
        }
...
fn main()
{
    ...
    for current in input_data.split('\n')
    {
        ...
        {
            map[current_last].push(Place{visited: false, value: current_value });
        }
    }
