    ...
    for i in 0..map.len()
    {
        for j in 0..map[i].len()
        {
            if start_pos.x as usize != i && start_pos.y as usize != j && map[i][j].freq == frequency
            {
                ...
                let antinode1 = Pos{x:start_pos.x-dist_x, y:start_pos.y-dist_y};

                if antinode1.x >= 0 && antinode1.x < map.len() as i32 && antinode1.y >= 0 && antinode1.y < map[i].len() as i32
                {
                    map[antinode1.x as usize][antinode1.y as usize].antinode = true;
                }

                let antinode2 = Pos{x:(i as i32)+dist_x, y:(j as i32)+dist_y};
